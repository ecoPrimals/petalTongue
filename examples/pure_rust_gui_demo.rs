...
async fn main() -> Result<()> {
    ...
    let backend_name = display.active_backend_name()
        .unwrap_or("None")
    ...
    while frame_count < target_frames {
        ...
        let output = ctx.run(Default::default(), |ctx| {
            egui::CentralPanel::default().show(ctx, |ui| {
                ...
                ui.horizontal(|ui| {
                    ui.label("Frame:");
                    ui.colored_label(egui::Color32::GREEN, format!("{}/{}", frame_count + 1, target_frames));
                });
        ...
        // Progress indicator
        if frame_count % 10 == 0 {
            println!("  Frame {}/{} ({:.2}ms)", frame_count, target_frames, frame_time.as_secs_f32() * 1000.0);
        }
    }
