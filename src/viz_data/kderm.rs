...
pub fn build_kderm_scene() -> SceneGraph {
    ...
    let layers = [
        KDermLayer {
            name: "extracellular",
            label: "Extracellular (Dark Forest)",
            color: Color::from_rgba8(49, 50, 68, 180),
            y: 10.0,
            height: 80.0,
        },
        KDermLayer {
            name: "outer",
            label: "Outer Membrane (Signal/Relay/Surface)",
            color: Color::from_rgba8(88, 91, 112, 200),
            y: 100.0,
            height: 80.0,
        },
        KDermLayer {
            name: "periplasm",
            label: "Periplasm (VPS relay, routing, telemetry)",
        ...
        KDermLayer {
            name: "plasma",
            label: "Plasma Membrane (Gate firewall)",
            color: Color::from_rgba8(137, 180, 250, 180),
            y: 290.0,
            height: 70.0,
        },
        KDermLayer {
            name: "cytoplasm",
            label: "Cytoplasm (Full NUCLEUS, 13 primals, UDS IPC)",
            color: Color::from_rgba8(166, 227, 161, 150),
            y: 370.0,
            height: 120.0,
        },
    ];
    ...
    for layer in &layers {
        let rect = Primitive::Rect {
            ...
            stroke: Some(StrokeStyle {
                color: Color::from_rgba8(205, 214, 244, 100),
                width: 1.0,
                cap: LineCap::Butt,
                join: LineJoin::Miter,
            }),
    ...
    for vps in &vps_nodes {
        let layer_y = layers
            .iter()
            .find(|l| l.name == vps.layer)
            .map_or(300.0, |l| l.y + l.height / 2.0);
    ...
    for (i, (from, fx, fy, to, tx, ty)) in relay_steps.iter().enumerate() {
        let arrow = Primitive::Line {
            ...
            stroke: StrokeStyle {
                color: Color::from_rgba8(243, 139, 168, 220),
                width: 2.5,
                cap: LineCap::Round,
                join: LineJoin::Round,
            },
            closed: false,
            data_id: Some(format!("relay-{i}")),
        };
        ...
        let step_node = SceneNode::new(format!("relay-step-{i}"))
            .with_primitive(arrow)
            .with_primitive(step_label)
    ...
    let bond_labels = [
        (
            ...
            700.0,
            430.0,
            Color::from_rgba8(166, 227, 161, 255),
        ),
        (
            "Metallic (relay)",
            700.0,
            235.0,
            Color::from_rgba8(249, 226, 175, 255),
        ),
        (
            "Ionic (cloud)",
            700.0,
        ...
        (
            ...
            700.0,
            50.0,
            Color::from_rgba8(186, 194, 222, 200),
        ),
    ];
