...
pub fn build_nucleus_scene() -> SceneGraph {
    ...
    let layers = [
        NucleusLayer {
            id: "full-nucleus", label: "Full NUCLEUS",
            components: &["+ Squirrel (AI)", "+ biomeOS (orchestration)", "+ petalTongue (UUI)"],
            color: Color::from_rgba8(203, 166, 247, 120),
            x: 50.0, y: 30.0, width: 700.0, height: 540.0,
        },
        NucleusLayer {
            id: "nest-atomic", label: "Nest Atomic",
            components: &["+ NestGate (storage)", "+ Provenance Trio"],
            color: Color::from_rgba8(148, 226, 213, 140),
            x: 80.0, y: 80.0, width: 640.0, height: 420.0,
        },
        NucleusLayer {
            id: "node-atomic", label: "Node Atomic",
            components: &["+ ToadStool (compute)", "+ barraCuda (GPU math)", "+ coralReef (GPU compiler)"],
            color: Color::from_rgba8(249, 226, 175, 140),
            x: 110.0, y: 130.0, width: 580.0, height: 310.0,
        },
        NucleusLayer {
            id: "tower-atomic", label: "Tower Atomic",
            components: &["BearDog (crypto)", "Songbird (network)", "= Pure Rust HTTPS"],
            color: Color::from_rgba8(166, 227, 161, 160),
            x: 140.0, y: 180.0, width: 520.0, height: 200.0,
        },
    ];
    ...
    for layer in &layers {
        let rect = Primitive::Rect {
            ...
            stroke: Some(StrokeStyle { color: Color::from_rgba8(205, 214, 244, 180), width: 2.0, cap: LineCap::Butt, join: LineJoin::Miter }),
            corner_radius: 8.0, data_id: Some(layer.id.to_string()),
        };
        let title = Primitive::Text {
            x: layer.x + 15.0, y: layer.y + 18.0, content: layer.label.to_string(),
        ...
        let mut node = SceneNode::new(layer.id)
            .with_primitive(rect).with_primitive(title).with_label(layer.label);

        for (ci, comp) in layer.components.iter().enumerate() {
            node.primitives.push(Primitive::Text {
                x: layer.x + layer.width - 15.0, y: layer.y + 18.0 + (ci as f64 * 16.0),
                content: (*comp).to_string(), font_size: 11.0,
                color: Color::from_rgba8(186, 194, 222, 220),
                anchor: AnchorPoint::TopRight, bold: false, italic: false, data_id: None,
            });
        }
    ...
    for (i, step) in tls_steps.iter().enumerate() {
        let text = Primitive::Text {
            x: 180.0, y: 220.0 + (i as f64 * 28.0), content: (*step).to_string(),
            font_size: 11.0, color: Color::from_rgba8(166, 227, 161, 240),
            anchor: AnchorPoint::TopLeft, bold: false, italic: false,
    ...
    let flow_label = SceneNode::new("flow-label").with_primitive(Primitive::Text {
        x: 400.0, y: 575.0,
        content: "Each layer adds capabilities. Tower is the minimum viable secure unit.".to_string(),
        font_size: 11.0, color: Color::from_rgba8(186, 194, 222, 200),
...
pub fn build_nucleus_expand_animation(layer_id: &str) -> Sequence {
    ...
    for (i, &lid) in layers[..=target_idx].iter().rev().enumerate() {
        ...
        animations.push(Animation {
            target: AnimationTarget::Scale { node_id: lid.to_string(), from: 1.0, to: 1.03 },
            duration_secs: 0.4, easing: Easing::EaseInOut, delay_secs: delay,
        });
        animations.push(Animation {
            target: AnimationTarget::Opacity { node_id: lid.to_string(), from: 0.6, to: 1.0 },
    ...
    for (i, &lid) in layers[..=target_idx].iter().rev().enumerate() {
        ...
        animations.push(Animation {
            target: AnimationTarget::Scale { node_id: lid.to_string(), from: 1.03, to: 1.0 },
            duration_secs: 0.3, easing: Easing::EaseIn, delay_secs: delay,
        });
    }
