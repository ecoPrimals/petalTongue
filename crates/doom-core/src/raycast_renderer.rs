...
use crate::wad_loader::{MapData, LineDef, Vertex};
...
impl RaycastRenderer {
    ...
    fn cast_ray(&self, map: &MapData, angle: f32) -> Option<(f32, [u8; 3])> {
        ...
        for linedef in &map.linedefs {
            ...
            if let Some(distance) = self.ray_line_intersection(
                self.player_x, self.player_y, ray_dx, ray_dy,
                v1, v2
            ) {
