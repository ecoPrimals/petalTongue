...
impl WadData {
    ...
    pub fn load<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        info!("WAD loaded successfully - Type: {:?}, Lumps: {}", header.wad_type, header.num_lumps);
        ...
        for (i, lump) in lumps.iter().enumerate() {
            if is_map_marker(&lump.name) {
                ...
                match parse_map(&lumps, i, &lump.name) {
                    Ok(map) => {
                        info!("Parsed map {} with {} vertices, {} linedefs", 
                              map.name, map.vertices.len(), map.linedefs.len());
                        maps.push(map);
                    }
                    Err(e) => {
    ...
    fn read_directory(file: &mut File, header: &WadHeader) -> Result<Vec<Lump>> {
        ...
        for _ in 0..header.num_lumps {
            ...
            let name_end = entry_bytes[8..16].iter()
                .position(|&b| b == 0)
                .unwrap_or(8);
