...
use crate::quality::{shannon_entropy, timing_entropy, variance, weighted_quality};
use crate::types::*;
...
impl AudioEntropyCapture {
    ...
    pub fn stop(&mut self) -> Result<()> {
        if self.state != CaptureState::Recording {
            bail!(
                "Cannot stop: capture is not recording (state: {:?})",
                self.state
            );
        }
    ...
    fn calculate_spectral_entropy(&self, buf: &AudioBuffer) -> f64 {
        ...
        let mut spectrum: Vec<Complex<f32>> = chunk.iter().map(|&s| Complex::new(s, 0.0)).collect();
    ...
    pub fn finalize(mut self) -> Result<AudioEntropyData> {
        ...
        if duration < MIN_CAPTURE_DURATION {
            warn!(
                "Finalizing with duration below minimum: {:.1}s",
                duration.as_secs_f64()
            );
        }
