...
use crate::flower::{FlowerState, FlowerAnimation};
    ...
    impl VisualFlowerRenderer {
        ...
        fn render_stem(&self, painter: &egui::Painter, center: Pos2, size: f32) {
            ...
            painter.line_segment(
                [stem_start, stem_end],
                Stroke::new(size * 0.04, stem_color),
            );
        }
        ...
        fn render_petal(
            ...
            let petal_center = center + Vec2::new(
                angle.cos() * length * 0.6,
                angle.sin() * length * 0.6,
            );
            
            painter.circle_filled(petal_center, width * 0.6, color_base);
            
            // Add highlight at tip
            let highlight_center = tip + Vec2::new(
                -angle.cos() * length * 0.15,
                -angle.sin() * length * 0.15,
            );
