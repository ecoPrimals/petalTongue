...
impl Visual2DRenderer {
    ...
    fn draw_stats(&self, ui: &mut egui::Ui, graph: &GraphEngine) {
        ...
        egui::Window::new("📊 Graph Statistics")
            ...
            .default_width(220.0)
            .collapsible(true)
            .frame(egui::Frame::window(&ui.ctx().style()).fill(egui::Color32::from_rgba_premultiplied(40, 40, 45, 230)))
            .show(ui.ctx(), |ui| {
                ...
                ui.label(egui::RichText::new(format!("Zoom: {:.2}x", self.zoom)).color(egui::Color32::from_rgb(150, 200, 255)));
                ...
                if let Some(selected_id) = &self.selected_node {
                    ...
                    ui.label(egui::RichText::new(selected_id).color(egui::Color32::from_rgb(255, 230, 150)));
                }
            });
    }
