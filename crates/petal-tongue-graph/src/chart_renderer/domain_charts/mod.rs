...
pub use validation::{scatter3d_bands, validate_scatter2d_lengths, validate_scatter3d_lengths};
...
pub fn draw_scatter(ui: &mut Ui, params: &Scatter2dParams<'_>) {
    ...
    let pts: Vec<[f64; 2]> = x_vals.iter().zip(y_vals).map(|(&x, &y)| [x, y]).collect();
