...
use petal_tongue_discovery::{discover_visualization_providers, HttpVisualizationProvider, VisualizationDataProvider};
...
async fn test_live_biomeos_integration() {
    ...
    assert!(primals_result.is_ok(), "Primal discovery should succeed: {:?}", primals_result.err());
    ...
    for primal in &primals {
        println!("     - {}: {} ({})", primal.id, primal.name, primal.primal_type);
        ...
        assert!(!primal.endpoint.is_empty(), "Primal endpoint should not be empty");
    ...
    assert!(topology_result.is_ok(), "Topology query should succeed: {:?}", topology_result.err());
    ...
    assert!(providers.is_ok(), "Auto-discovery should succeed: {:?}", providers.err());
    ...
    assert!(discovered_primals.is_ok(), "Query through discovered provider should work");
    println!("   ✅ Queried through discovered provider: {} primals", discovered_primals.unwrap().len());
...
async fn test_biomeos_api_contract() {
    ...
    use petal_tongue_core::{PrimalInfo, PrimalHealthStatus};
    use serde_json::json;
