...
impl SongbirdClient {
    ...
    pub async fn discover_by_capability(&self, capability: &str) -> Result<Vec<PrimalInfo>> {
        ...
        for primal in primals {
            if let Ok(info) = self.parse_primal(primal) {
                ...
                warn!("Failed to parse primal from Songbird response: {:?}", primal);
            }
        }
        
        info!("🎵 Songbird found {} primals with capability '{}'", 
             primal_infos.len(), capability);
    ...
    pub async fn get_all_primals(&self) -> Result<Vec<PrimalInfo>> {
        ...
        info!("🎵 Songbird reports {} total registered primals", primal_infos.len());
    ...
    async fn send_request(&self, request: Value) -> Result<Value> {
        ...
        if let Some(error) = response.error {
            anyhow::bail!("Songbird returned error: {} (code: {})", error.message, error.code);
        }
        
        response.result.ok_or_else(|| anyhow::anyhow!("No result in Songbird response"))
    }
