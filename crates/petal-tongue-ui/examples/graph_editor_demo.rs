...
async fn main() -> Result<()> {
    ...
    info!("✅ Created graph with {} nodes and {} edges", 
        graph.get_nodes().len(), 
        graph.get_edges().len()
    );
    ...
    info!("✅ Saved template: {} (ID: {})\n", 
        "Data Pipeline", 
        template_resp.template_id
    );

    // 5. Simulate graph execution with streaming updates
    info!("🚀 Simulating graph execution...");
    stream_handler.start_execution("demo-graph".to_string()).await?;

    // Spawn a task to listen for stream messages
    let _listener = tokio::spawn(async move {
        while let Ok(msg) = rx.recv().await {
            match msg {
                petal_tongue_ui::graph_editor::StreamMessage::NodeStatus { node_id, status, .. } => {
                    info!("  📡 Node '{}' status: {:?}", node_id, status);
                }
                petal_tongue_ui::graph_editor::StreamMessage::Progress { node_id, progress, message, .. } => {
                    info!("  📊 Node '{}' progress: {:.0}% - {}", node_id, progress * 100.0, message);
                }
                petal_tongue_ui::graph_editor::StreamMessage::Reasoning { reasoning, .. } => {
                    info!("  🤖 AI Decision: {} (confidence: {:.0}%)", 
                        reasoning.decision, 
    ...
    for node_id in &execution_order {
        // Start node
        stream_handler.send_node_status(
    ...
    let reasoning = petal_tongue_ui::graph_editor::streaming::AIReasoning {
        ...
        alternatives: vec![
            petal_tongue_ui::graph_editor::streaming::Alternative {
        ...
        patterns: vec![
            petal_tongue_ui::graph_editor::streaming::Pattern {
    ...
    stream_handler.send_reasoning("demo-graph".to_string(), reasoning).await?;
