...
fn test_e2e_minimal_fallback_scenario_complete_flow() {
    ...
    for node in graph.nodes() {
        ...
        assert!(!node.info.primal_type.is_empty(), "Node type should not be empty");
        assert!(!node.info.endpoint.is_empty(), "Node endpoint should not be empty");
    }
...
fn test_e2e_tutorial_self_awareness() {
    ...
    assert!(self_node.is_some(), "Tutorial should include petalTongue itself");
    
    let self_node = self_node.unwrap();
    assert_eq!(self_node.info.primal_type, "Visualization");
    assert!(self_node.info.capabilities.contains(&"visual_2d".to_string()));
}
