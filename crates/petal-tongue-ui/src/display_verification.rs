...
impl DisplayVerification {
    ...
    pub fn confirmed_visible() -> Self {
        Self {
            ...
            status_message: "Output confirmed reaching viewer - user actively interacting".to_string(),
            suggested_action: None,
        }
    }
    ...
    pub fn failed(reason: &str) -> Self {
        Self {
            ...
            suggested_action: Some("Check DISPLAY environment variable and display server".to_string()),
        }
    }
}
...
pub fn detect_display_topology() -> (DisplayTopology, Vec<String>) {
    ...
    } else if evidence.iter().any(|e| e.contains("forwarding") || e.contains("localhost")) {
        DisplayTopology::Forwarded
    } else if evidence.iter().any(|e| e.contains("virtual") || e.contains("Xvfb")) {
...
pub fn verify_display_substrate(window_title: &str) -> DisplayVerification {
    ...
    match (window_found, wm_responsive, &topology) {
        ...
        (_, _, DisplayTopology::Virtual) => {
            ...
            let mut verif = DisplayVerification::probable(topology.clone(), evidence);
            verif.status_message = "Virtual display detected - no physical viewer expected".to_string();
            verif
        }
        ...
        (true, _, DisplayTopology::Unknown) => {
            ...
            let mut verif = DisplayVerification::probable(topology.clone(), evidence);
            verif.status_message = "Unknown display topology, but window found - probable visibility".to_string();
            verif
        }
        ...
        _ => {
            ...
            let mut verif = DisplayVerification::probable(topology.clone(), evidence);
            verif.status_message = "Display topology unclear and window not found - uncertain".to_string();
            verif
        }
    }
}
...
pub fn continuous_verification(window_title: &str, last_interaction_secs: f32) -> DisplayVerification {
    ...
    if verification.interactivity == InteractivityState::Active {
        ...
        verification.status_message = match verification.display_topology {
            DisplayTopology::DirectLocal => "Direct local display - user actively interacting".to_string(),
            DisplayTopology::Forwarded => "Forwarded display confirmed - user interaction proves visibility".to_string(),
            DisplayTopology::Nested => "Nested display confirmed - user interaction proves visibility".to_string(),
            DisplayTopology::Virtual => "Virtual display with active interaction (unusual but confirmed)".to_string(),
            DisplayTopology::Unknown => "Display topology unknown, but user interaction confirms visibility".to_string(),
        };
        ...
        match verification.display_topology {
            ...
            DisplayTopology::DirectLocal => {
                ...
                if verification.window_exists {
                    verification.visibility = VisibilityState::Probable;
                    verification.status_message = "Direct local display - window exists but no recent interaction".to_string();
                }
            }
            DisplayTopology::Virtual => {
                // Virtual displays don't expect interaction
                verification.visibility = VisibilityState::Probable;
                verification.status_message = "Virtual display - no human interaction expected".to_string();
            }
        }
    }
