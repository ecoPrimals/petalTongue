...
impl OutputVerification {
    ...
    pub fn confirm_via_interaction(&mut self) {
        ...
        self.evidence.push("User interaction confirms perception".to_string());
    }
    ...
    pub fn confirm_via_device_ack(&mut self, device_info: String) {
        ...
        self.evidence.push(format!("Device acknowledgment: {}", device_info));
    }
...
impl OutputVerificationSystem {
    ...
    pub fn register_output(&mut self, modality: OutputModality) {
        if !self.verifications.contains_key(&modality) {
            ...
            self.verifications.insert(modality.clone(), OutputVerification::unverified(modality));
        }
    }
    ...
    pub fn get_status_summary(&self) -> String {
        ...
        let confirmed = self.verifications.values().filter(|v| v.reaches_user).count();
        let stale = self.verifications.values().filter(|v| v.is_stale(Duration::from_secs(30))).count();
        ...
        for (modality, verification) in &mut self.verifications {
            if verification.is_stale(Duration::from_secs(300)) {
                ...
                verification.suggested_action = Some(
                    "Interact with the application to confirm you can perceive this output".to_string()
                );
            }
        }
    }
...
pub fn detect_audio_topology() -> (OutputTopology, Vec<String>) {
        ...
        .unwrap_or(false)
    {
        ...
        if let Ok(output) = std::process::Command::new("pactl").arg("list").arg("sinks").output() {
...
pub fn detect_haptic_topology() -> (OutputTopology, Vec<String>) {
    ...
    if let Ok(entries) = std::fs::read_dir("/dev/input") {
        ...
        if !ff_devices.is_empty() {
            evidence.push(format!("Found {} input devices (may support haptic)", ff_devices.len()));
        }
    }
