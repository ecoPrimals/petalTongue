...
impl TrustDashboard {
            ...
            if let Some(trust_value) = primal.properties.get("trust_level") {
                ...
                *summary.trust_distribution.entry(trust_label).or_insert(0) += 1;
                ...
                if let Some(trust_level) = primal.trust_level {
                    trust_values.push(trust_level as f64);
    ...
    pub fn render(&mut self, ui: &mut Ui, palette: &ColorPalette, font_scale: f32, audio_system: Option<&AudioSystem>) {
        ui.heading(RichText::new("🔐 Trust Dashboard").size(16.0 * font_scale).strong());
        ui.add_space(8.0);

        // Trust distribution section
        ui.label(RichText::new("Network Trust Distribution").size(14.0 * font_scale).strong());
        ui.add_space(4.0);

        if self.trust_summary.total_primals == 0 {
            ui.label(RichText::new("No primals discovered yet").size(12.0 * font_scale).color(palette.text_dim));
            return;
        }

        // Total primals
        ui.horizontal(|ui| {
            ui.label(RichText::new("Total Primals:").size(12.0 * font_scale).color(palette.text_dim));
            ui.label(RichText::new(format!("{}", self.trust_summary.total_primals))
        ...
        for (level, count) in trust_levels {
            ...
            ui.horizontal(|ui| {
                ...
                ui.label(RichText::new(format!("{} ({:.0}%)", count, percentage))
                    .size(12.0 * font_scale)
                    .color(palette.text));
            });
        }
        ...
        if let Some(avg) = self.trust_summary.average_trust {
            ui.label(RichText::new("Average Trust Level").size(14.0 * font_scale).strong());
            ui.add_space(4.0);
            ...
            ui.horizontal(|ui| {
                ...
                ui.label(RichText::new(format!("{:.2}", avg)).size(16.0 * font_scale).color(color).strong());
                ui.label(RichText::new(format!("({})", label)).size(12.0 * font_scale).color(palette.text_dim));
            });
        ...
        ui.separator();
        ui.add_space(8.0);
        ui.label(RichText::new("🌳 Family Relationships").size(14.0 * font_scale).strong());
        ui.add_space(4.0);

        ui.horizontal(|ui| {
            ui.label(RichText::new("Primals with Family:").size(12.0 * font_scale).color(palette.text_dim));
            ui.label(RichText::new(format!("{}", self.trust_summary.family_count))
                .size(12.0 * font_scale)
                .strong());
        });

        ui.horizontal(|ui| {
            ui.label(RichText::new("Unique Families:").size(12.0 * font_scale).color(palette.text_dim));
            ui.label(RichText::new(format!("{}", self.trust_summary.unique_families))
                .size(12.0 * font_scale)
                .strong());
        });
        ...
        ui.label(RichText::new(format!("Updated {} seconds ago", elapsed))
    ...
    pub fn render_compact(&self, ui: &mut Ui, palette: &ColorPalette, font_scale: f32) {
        ui.horizontal(|ui| {
            ...
            if let Some(avg) = self.trust_summary.average_trust {
                ...
                ui.label(RichText::new(format!("{:.1}", avg))
                    .size(12.0 * font_scale)
                    .color(color));
            } else {
                ui.label(RichText::new("N/A").size(12.0 * font_scale).color(palette.text_dim));
            }

            if self.trust_summary.total_primals > 0 {
                ui.label(RichText::new(format!("({}/{})", 
                    self.trust_summary.family_count,
                    self.trust_summary.total_primals))
                    .size(10.0 * font_scale)
                    .color(palette.text_dim));
            }
        });
...
mod tests {
    ...
    fn create_test_primal(id: &str, trust: Option<u8>, family: Option<&str>) -> PrimalInfo {
        ...
        if let Some(f) = family {
            props.insert("family_id".to_string(), PropertyValue::String(f.to_string()));
        }
    ...
    fn test_trust_distribution() {
        ...
        assert_eq!(dashboard.trust_summary.trust_distribution.get("Full (3)"), Some(&2));
        assert_eq!(dashboard.trust_summary.trust_distribution.get("Elevated (2)"), Some(&1));
        assert_eq!(dashboard.trust_summary.trust_distribution.get("Limited (1)"), Some(&1));
    }
