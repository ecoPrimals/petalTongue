...
impl UniversalDiscovery {
    ...
    pub async fn discover_capability(&self, capability: &str) -> Result<Vec<DiscoveredService>> {
        info!("🔍 Discovering capability: '{}' (infant mode - zero assumptions)", capability);
        ...
        if discovered.is_empty() {
            ...
            info!("✅ Total discovered: {} service(s) for '{}'", discovered.len(), capability);
        }
    ...
    async fn discover_via_environment(&self, capability: &str) -> Result<Vec<DiscoveredService>> {
        ...
        if let Ok(discovery_endpoint) = std::env::var("DISCOVERY_SERVICE_ENDPOINT") {
            ...
            if let Ok(discovered_services) = self.query_discovery_service(&discovery_endpoint, capability).await {
                ...
                for entry in entries.flatten() {
                    if let Some(path) = entry.path().to_str() {
                        if path.ends_with(".sock") {
                            ...
                            if let Ok(socket_services) = self.query_generic_endpoint(&endpoint, capability).await {
    ...
    ///
    /// Probes common ports WITHOUT assumptions:
    /// - localhost:8080-8090 (common service ports)
    ...
    async fn discover_via_http(&self, capability: &str) -> Result<Vec<DiscoveredService>> {
        ...
        // Common service ports (but NO service names!)
...
mod tests {
    ...
    async fn test_discover_capability_no_results() {
        let discovery = UniversalDiscovery::new();
        let results = discovery.discover_capability("nonexistent-capability").await;
