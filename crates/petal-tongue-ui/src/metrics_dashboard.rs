...
use egui::{Color32, ProgressBar, RichText, Ui, Vec2, Stroke};
...
impl MetricsDashboard {
        ...
        match provider.get_metrics().await {
            Ok(result) => {
                ...
                match serde_json::from_value::<SystemMetrics>(result) {
                    Ok(metrics) => {
                        debug!("Metrics data received: CPU {:.1}%, Mem {:.1}%", 
    ...
    pub fn render(&self, ui: &mut Ui) {
        ...
        if let Some(data) = &self.data {
            ...
            ui.label(RichText::new("No metrics data available")
    ...
    fn render_cpu_section(&self, ui: &mut Ui, data: &SystemMetrics) {
        ui.group(|ui| {
            ...
            if !self.cpu_history.values().is_empty() {
                ui.horizontal(|ui| {
                    ui.label(RichText::new(format!("Avg: {:.1}%", self.cpu_history.average()))
                        .color(Color32::from_rgb(156, 163, 175))); // gray-400
                    ui.label(RichText::new(format!("Max: {:.1}%", self.cpu_history.max()))
    ...
    fn render_system_info(&self, ui: &mut Ui, data: &SystemMetrics) {
        ui.group(|ui| {
            ...
            ui.horizontal(|ui| {
                ui.label("⏱️ Uptime:");
                ui.label(RichText::new(data.uptime_formatted())
    ...
    fn render_neural_api_info(&self, ui: &mut Ui, data: &SystemMetrics) {
        ui.group(|ui| {
            ...
            ui.horizontal(|ui| {
                ui.label("🧬 Family:");
                ui.label(RichText::new(&data.neural_api.family_id)
                    .color(Color32::from_rgb(168, 85, 247))); // purple-500
            });
            
            ui.horizontal(|ui| {
                ui.label("🌸 Active Primals:");
                ui.label(RichText::new(format!("{}", data.neural_api.active_primals))
                    .strong()
            ...
            ui.horizontal(|ui| {
                ...
                ui.label(RichText::new(format!("{}", data.neural_api.active_executions))
                    .color(color));
            });
        });
    }
