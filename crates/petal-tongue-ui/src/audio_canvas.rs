...
impl AudioCanvas {
    ...
    pub fn discover() -> Result<Vec<PathBuf>> {
        ...
        for entry in std::fs::read_dir(snd_dir)
            .context("Failed to read /dev/snd directory")?
        {
            let path = entry?.path();
            let name = path
                .file_name()
    ...
    pub fn open(device_path: &Path) -> Result<Self> {
        ...
        let device = OpenOptions::new()
            .write(true)
            .open(device_path)
            .context(format!("Failed to open audio device: {}", device_path.display()))?;
...
mod tests {
    ...
    fn test_open_and_write() {
        ...
        let mut canvas = AudioCanvas::open(&devices[0])
            .expect("Failed to open audio canvas");
        ...
        canvas.write_samples(&samples).expect("Failed to write samples");
