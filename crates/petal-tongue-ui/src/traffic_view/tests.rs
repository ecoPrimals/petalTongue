...
use super::view::{
    TrafficIntent, bezier_control_points, calculate_flow_color, calculate_flow_width,
...
fn test_flow_width_calculation() {
    ...
    let max_vol = view
        ...
        .max()
        .unwrap_or(1);
    let width1 = super::view::calculate_flow_width(&view.flows()[0].metrics, max_vol, 2.0, 40.0);
    let width2 = super::view::calculate_flow_width(&view.flows()[1].metrics, max_vol, 2.0, 40.0);
...
fn test_flow_width_single_flow() {
    ...
    let max_vol = view
        ...
        .max()
        .unwrap_or(1);
    let width = super::view::calculate_flow_width(&view.flows()[0].metrics, max_vol, 2.0, 40.0);
    assert!(width >= 2.0);
    assert!(width <= 40.0);
}
