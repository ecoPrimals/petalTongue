...
impl InputVerification {
    ...
    pub fn record_input(&mut self) {
        ...
        self.status_message = format!(
            "{:?} input active - receiving signals",
            self.modality
        );
...
impl InputVerificationSystem {
    ...
    pub fn register_input(&mut self, modality: InputModality) {
        if !self.verifications.contains_key(&modality) {
            ...
            self.verifications.insert(modality.clone(), InputVerification::unverified(modality));
        }
    }
    ...
    pub fn get_status_summary(&self) -> String {
        ...
        let active = self.verifications.values().filter(|v| v.input_active).count();
    ...
    pub fn update(&mut self) {
        ...
        for (modality, verification) in &mut self.verifications {
            ...
            if verification.is_stale(Duration::from_secs(300)) {
                warn!(
                    "⚠️  {:?} input is stale (no recent input)",
                    modality
                );
                verification.status_message = format!(
                    "{:?} input: No recent activity detected",
                    modality
                );
                verification.suggested_action = Some(
                    format!("Try using {:?} input to confirm it's working", modality)
                );
            }
        }
    }
