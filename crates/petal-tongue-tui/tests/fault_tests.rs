...
mod common;
use common::{create_test_primal, create_test_primal_with_health, create_test_edge};
...
async fn test_fault_unknown_health_status() {
    ...
    let primals = vec![
        create_test_primal_with_health("unknown-primal", "unknown-1", PrimalHealthStatus::Unknown),
    ];
    ...
    assert!(matches!(retrieved_primals[0].health, PrimalHealthStatus::Unknown));
}
...
async fn test_fault_critical_health_status() {
    ...
    let primals = vec![
        create_test_primal_with_health("failing-primal", "failing-1", PrimalHealthStatus::Critical),
    ];
    ...
    let retrieved_primals = state.get_primals().await;
    assert_eq!(retrieved_primals.len(), 1);
    assert!(matches!(retrieved_primals[0].health, PrimalHealthStatus::Critical));
}
...
async fn test_fault_invalid_topology_references() {
    ...
    // Add primals
    let primals = vec![
        create_test_primal("primal-1", "primal-1"),
    ];
...
async fn test_fault_concurrent_primal_access() {
    ...
    let writer = tokio::spawn(async move {
        for i in 0..100 {
            let primals = vec![
                create_test_primal(&format!("primal-{}", i), &format!("primal-{}", i)),
            ];
