...
mod common;
use common::{create_test_primal, create_test_edge};
        ...
        for iteration in 0..100 {
            ...
            let topology: Vec<_> = (0..10)
                .map(|i| create_test_edge(
                    ...
                    if iteration % 2 == 0 { "discovery" } else { "compute" },
                ))
                .collect();
        ...
        let handle = tokio::spawn(async move {
            for i in 0..500 {
                state_clone.add_log(LogMessage {
                    timestamp: Utc::now(),
                    source: Some(format!("task-{}", task_id)),
                    level: if i % 5 == 0 { LogLevel::Error } else { LogLevel::Info },
    ...
    for primal_id in 0..50 {
        ...
        let handle = tokio::spawn(async move {
            for iteration in 0..20 {
                ...
                state_clone.register_capability(
                    format!("primal-{}", primal_id),
                    capabilities,
                );
            }
        });
