...
| Metric | Status |
|--------|--------|
| Tests | 6,347+ passing, 0 failures (`--all-features`) |
...
| Error handling | Typed `thiserror` errors throughout -- zero `anyhow`, zero `Result<_, String>`, typed `#[from]` chains (`AppError`, `ContentBackendError`); zero `"literal".to_string()` in production |
...
cargo test --workspace --all-features        # 6,347+ tests
